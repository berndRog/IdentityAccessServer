...
) : BaseApiClient<OwnerClient>(factory, json, logger)
{
   ...
   public Task<Result<OwnerProvisionDto>> PostProvisionAsync(CancellationToken ct = default) 
      => SendAsync<OwnerProvisionDto>(
         () => _http.PostAsync($"{MeBase}/provisioned", content: null, ct), ct);

   // GET bankingapi/v1/owners/me/profile -> 200 OK + OwnerProfileDto
   public Task<Result<OwnerProfileDto>> GetProfileAsync(CancellationToken ct = default) 
      => SendAsync<OwnerProfileDto>(() => _http.GetAsync($"{MeBase}/profile", ct), ct);

   // PUT bankingapi/v1/owners/me/profile -> 200 OK + OwnerProfileDto
   public Task<Result<OwnerProfileDto>> UpdateProfileAsync(
      OwnerProfileDto dto,
      CancellationToken ct = default
   ) => SendAsync<OwnerProfileDto>(
